defmodule CollabCanvas.Performance.CanvasLoadTest do
  ...
  setup do
    ...
    # Create test canvas
    {:ok, canvas} = Canvases.create_canvas(%{
  ...
  test "maintains > 45 FPS with 2000 objects", %{canvas: canvas, user: user} do
    ...
    objects = Enum.map(1..@object_count, fn i ->
      ...
      {:ok, object} = Canvases.create_object(canvas.id, %{
        type: "rectangle",
        position: %{x: x, y: y},
        data: %{
          width: 15,
  ...
  test "object sync latency stays below 150ms under load", %{canvas: canvas, user: user} do
    ...
    latencies = Enum.map(1..100, fn i ->
      ...
      after
        1000 -> 1000 # Timeout after 1 second
      end
    end)
  ...
  test "handles 10 concurrent users editing simultaneously", %{canvas: canvas, user: _user} do
    ...
    IO.puts("  Throughput: #{Float.round(length(all_objects) / (total_time / 1000), 2)} objects/sec")
  ...
  test "generates performance report", %{canvas: canvas, user: user} do
    ...
    report_path = "test/performance/reports/performance_report_#{DateTime.to_unix(report.timestamp)}.json"
