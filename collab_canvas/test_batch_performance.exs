...
user = case Repo.get_by(Accounts.User, email: user_attrs.email) do
  nil ->
    %Accounts.User{}
...
attrs_list_10 = Enum.map(0..9, fn i ->
  %{
    type: "rectangle",
...
{time_10, {:ok, objects_10}} = :timer.tc(fn ->
...
attrs_list_50 = Enum.map(0..49, fn i ->
  %{
    type: "circle",
...
{time_50, {:ok, objects_50}} = :timer.tc(fn ->
...
attrs_list_100 = Enum.map(0..99, fn i ->
  %{
    type: "rectangle",
...
{time_100, {:ok, objects_100}} = :timer.tc(fn ->
...
IO.puts("\n=== Performance Summary ===")
IO.puts("10 objects:  #{Float.round(time_10_ms, 2)}ms (#{Float.round(10000/time_10_ms, 2)} obj/sec)")
IO.puts("50 objects:  #{Float.round(time_50_ms, 2)}ms (#{Float.round(50000/time_50_ms, 2)} obj/sec)")
IO.puts("100 objects: #{Float.round(time_100_ms, 2)}ms (#{Float.round(100000/time_100_ms, 2)} obj/sec)")
