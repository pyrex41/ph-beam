defmodule CollabCanvas.AI.BatchProcessor do
  ...
  def execute_batched_creates(create_calls, canvas_id, current_color, normalize_color_fun) do
    ...
    {all_attrs, call_metadata} =
      Enum.reduce(create_calls, {[], []}, fn tool_call, {attrs_acc, meta_acc} ->
        # Transform tool call into object attributes
        {new_attrs, object_count} =
          build_object_attrs_from_tool_call(tool_call, current_color, normalize_color_fun)
  ...
  """
  def build_object_attrs_from_tool_call(
        %{name: "create_shape", input: input},
        current_color,
        normalize_color_fun
      ) do
  ...
  def combine_results_in_order(original_tool_calls, batch_results, other_results) do
    ...
    {results, _} =
      Enum.reduce(original_tool_calls, {[], state}, fn tool_call, {results_acc, current_state} ->
